...
export default {
  ...
  async beforeRouteUpdate(to) {
    await this.handleLoadSequence(to.params.classId, to.params.taskId);
  },
  methods: {
    async handleLoadSequence(classId, taskId) {
      // update page state
      this.loading = true;
      ...
      this.taskResponses = taskResponses.map((x) => ({
        ...
        student: {
          ...
          name:
            x?.student && x?.student?.name ? x?.student?.name : "student-name",
          avatar:
